...
const kv={async delete(){calls.push('clear')},async put(){calls.push('save')},async get(){return null}};
for(const [response,expectLog] of [[new Response('{"ok":true,"id":1}'),false],[new Response('{}',{status:500}),true],[new Response('{"ok":false}'),true],[new Response('not json'),true],[new Error('timeout'),true]]){calls=[];logs=[];syncResponse=response;await context.handleTrialInput({replyToken:'fake'}, {COUPONS:kv},'fake',{step:'address',name:'測試',phone:'0912345678',product:'test'},'台北市測試地址','fake-user');assert(calls[0].includes('/reply'));assert.equal(calls.filter(x=>x.includes('/reply')).length,1);assert.equal(logs.some(x=>x.includes('sync to cash-bio failed')),expectLog);}
console.log('PASS: Taiwan date boundaries, invalid dates, 507-row pagination, filters, auth, admin script, LINE success/HTTP failure/bad JSON/timeout and reply-first ordering.');
